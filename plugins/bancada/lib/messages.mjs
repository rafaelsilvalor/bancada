...
const CATALOG = {
  en: {
    ...
    "doctor.colocated.title": () => "Test colocation",
    "doctor.colocated.coverage": ({ tested, total, excepted, missing }) =>
      `${tested} of ${total} module(s) have a test; ${excepted} excepted, ${missing} missing`,
    "doctor.colocated.off": () => "the colocated gate is off, so this is a report, not a boundary",
    "doctor.colocated.missing": ({ file, candidate }) => `missing  ${file} — expected ${candidate}`,
    "doctor.colocated.more": ({ n }) => `… ${n} more; bancada doctor --json carries the full list`,
    "doctor.colocated.deadSuite": ({ test }) => `suite test does not exist  ${test} — it covers nothing`,
    "doctor.colocated.stale": ({ path }) => `stale exception  ${path} — the file no longer exists`,
    "doctor.colocated.unneeded": ({ path }) => `exception no longer needed  ${path} — a test now covers it`,
  ...
  "pt-BR": {
    ...
    "doctor.colocated.title": () => "Colocalização de testes",
    "doctor.colocated.coverage": ({ tested, total, excepted, missing }) =>
      `${tested} de ${total} módulo(s) têm teste; ${excepted} em exceção, ${missing} faltando`,
    "doctor.colocated.off": () => "o gate colocated está desligado; isto é relatório, não fronteira",
    "doctor.colocated.missing": ({ file, candidate }) => `faltando  ${file} — esperado ${candidate}`,
    "doctor.colocated.more": ({ n }) => `… mais ${n}; a lista completa sai em bancada doctor --json`,
    "doctor.colocated.deadSuite": ({ test }) => `teste da suíte não existe  ${test} — não cobre nada`,
    "doctor.colocated.stale": ({ path }) => `exceção obsoleta  ${path} — o arquivo não existe mais`,
    "doctor.colocated.unneeded": ({ path }) => `exceção desnecessária  ${path} — um teste agora cobre o arquivo`,
